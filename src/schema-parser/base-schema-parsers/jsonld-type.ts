  ...
  override parse() {
    ...
    if (
      ...
      return {
        ...
        $parsedSchema: true,
        schemaType: SCHEMA_TYPES.JSONLD_TYPE,
        // A JSON-LD type resolves to a string-literal union alias, not an
        // enum declaration — claiming `ENUM` here would route the schema to
        // the enum formatter, which expects `content` to be a list of members.
        type: SCHEMA_TYPES.PRIMITIVE,
